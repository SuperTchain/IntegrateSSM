...
public class LogUserActions {
     ...
     * 日志监控
     */
    private static Logger logger = Logger.getLogger(LogUserActions.class);

    /**
     * 抽象切点方法 到时候只代理添加注解的方法
     */
    @Pointcut("@annotation(com.lx.annotation.LogAnnotation)")
    public void pt1(){};
    ...
    public void logMessage(JoinPoint joinPoint)  {
        ...
        if(clazz.isAnnotationPresent(LogAnnotation.class)){
            // 当前controller操作的名称
            controllerOperation = clazz.getAnnotation(LogAnnotation.class).name();
        }
        //获取当前方法
        ...
        for (Method m : methods) {
            if(m.equals(method)){
                ...
                if(m.isAnnotationPresent(LogAnnotation.class)){
                    methodOperation = m.getAnnotation(LogAnnotation.class).name();
                    urlOperation = m.getAnnotation(LogAnnotation.class).url();
                }
            }
        }
